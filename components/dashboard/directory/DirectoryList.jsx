...
import { useSessionInfo } from "../../../context/GlobalContext";
export default function DirectoryList({newProfile, setNewProfile, newDirectory: directory,setNewDirectory }) {
  
  
  function mostrarMiembro(person) {
    const mostrado = directory.filter((member) => member.id == person);
    setNewProfile(mostrado);
  }

  const {member, currentMember, setCurrentMember}= useSessionInfo()
  console.log('Context Global', member)
  
  //console.log('nuevoPerfil',newProfile)
  return (
        ...
        <div className="relative">
          <ul role="list" className="relative z-0 divide-y divide-gray-200">
            {directory.map((person) => (
              <li onClick={() => setCurrentMember(person)} key={person.id}>
                <div className="relative flex items-center space-x-3 px-6 py-5 focus-within:ring-2 focus-within:ring-inset focus-within:ring-pink-500 hover:bg-gray-50">
                  ...
                  <div className="min-w-0 flex-1">
                    <a
                      ...
                      onClick={() => mostrarMiembro(person.id) }
                      href="#"
